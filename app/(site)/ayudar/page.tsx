    ...
    <main className="site-shell">
      ...
      <section className="activities-section page-section" aria-labelledby="activities-title">
        <div className="activities-inner">
          ...
          <div className={`activity-list-clean${activities.length > 6 ? ' is-scrollable' : ''}`}>{activities.map((activity) => <article className={`activity-row${activity.featured ? ' is-featured' : ''}`} key={activity.id}><div className="activity-date"><strong>{String(activity.date).split(' ')[0]}</strong><small>{String(activity.date).includes('Mañana') ? 'AGO' : 'HOY'}</small></div><div>{activity.featured && <span className="featured-badge">Destacado</span>}<h3>{activity.title}</h3><p>{activity.time} · {activity.location}</p></div><strong className="activity-spots">{activity.spots}</strong></article>)}</div>
