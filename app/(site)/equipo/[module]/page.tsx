...
export default async function StaffModulePage({ params }: PageProps) {
  ...
  const records = JSON.parse(JSON.stringify(result.docs)) as Record<string, unknown>[]
  const modules = getModulesForRole(session.user.role)

  return <PortalShell name={session.user.name} role={session.user.role} modules={modules}><section className="staff-page-intro"><div><Link className="staff-back-link" href="/equipo">← Volver al resumen</Link><p className="staff-eyebrow">Módulo operativo</p><h1>{moduleDefinition.label}</h1><p>{moduleDefinition.description}</p></div></section>{showAllRequests ? <SupportRequestPanel initialRecords={records} canManage /> : <StaffModulePanel module={moduleForView} initialRecords={records} initialPage={result.page || 1} initialTotalPages={result.totalPages || 1} initialTotalDocs={result.totalDocs} />}</PortalShell>
}
